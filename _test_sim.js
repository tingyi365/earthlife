...
/* ===== R136 人際羈絆網・人生重要他人（純呈現/唯讀疊加層：零 rng/零 pinned 序列擾動/deterministic/降級） ===== */
let r136OK = false;
try {
  const r136Raw = vm.runInContext(`(function(){
    const out={};
    /* ① 結構：>=9 NPC、五個羈絆成就齊備 */
    out.struct = Array.isArray(R136_PEOPLE) && R136_PEOPLE.length>=9
      && R136_PEOPLE.every(p=>p.k&&p.ic&&p.nm&&p.role&&p.grp&&typeof p.d==='function')
      && ['r136_full','r136_lonely','r136_drag','r136_owe','r136_cutoff'].every(id=>!!ACH_MAP[id]);
    /* ② 唯讀疊加零 rng：equipped 與否無關——startGame 後跑 r136Roster/Review 不消耗任何種子序列 */
    const oc=randomSeedCode; randomSeedCode=function(){return 'DDDDDD';};
    startGame(); const r0=[]; for(let i=0;i<20;i++)r0.push(rng());
    startGame(); r136Roster(); r136PeopleReviewHTML(); const r1=[]; for(let i=0;i<20;i++)r1.push(rng());
    ...
    startGame(); S.attr={hp:80,int:75,apr:70,mny:72,hap:78}; S.age=60; S.bonds={boss:20,lover:30}; S.flags={};
    const snap=JSON.stringify(S.attr)+'|'+JSON.stringify(S.bonds)+'|'+S.age;
    const c1=r136PeopleReviewHTML(), c2=r136PeopleReviewHTML();
    out.deterministic = c1===c2 && c1.indexOf('人生重要他人')>=0
      && (JSON.stringify(S.attr)+'|'+JSON.stringify(S.bonds)+'|'+S.age)===snap;
    /* ④ S=null/缺 attr 安全降級：roster 空、卡空字串、不炸 */
    const sv=S; S=null; let nullSafe=true;
    try{ if(r136Roster().length!==0) nullSafe=false; if(r136PeopleReviewHTML()!=='') nullSafe=false; }catch(e){ nullSafe=false; }
    S=sv; out.nullSafe = nullSafe;
    return JSON.stringify(out);
  })()`, sandbox);
  const r136 = JSON.parse(r136Raw);
  r136OK = Object.values(r136).every(v => v === true);
  console.log(`R136 人際羈絆網・人生重要他人: ${r136OK ? '✅ 全數通過' : '❌ ' + JSON.stringify(r136)}`);
} catch (e) {
  console.log('R136 人際羈絆網・人生重要他人: ❌ ' + e.message);
}

/* ===== R138 健康人生軌跡・病歷與長照（純衍生/結算覆蓋層：零 rng/零 pinned 序列擾動/deterministic/健康主驅動/降級） ===== */
let r138OK = false;
try {
  const r138Raw = vm.runInContext(`(function(){
    const out={};
    /* ① 結構：6 健康階層、6 結局型別、五個健康成就＋集滿成就齊備 */
    out.struct = Array.isArray(R138_TIERS) && R138_TIERS.length===6 && Array.isArray(R138_PHASES) && R138_PHASES.length===4
      && ['ironman','peaceful','chronic','er','karoshi','longcare'].every(k=>R138_END[k])
      && ['r138_ironman','r138_peaceful','r138_pillbox','r138_karoshi','r138_longcare','r138_health'].every(id=>!!ACH_MAP[id]);
    /* ② 純衍生覆蓋零 rng：startGame 後跑 r138HealthOf/Review 不消耗任何種子序列（與 pinned 抽事件序列獨立） */
    const oc=randomSeedCode; randomSeedCode=function(){return 'DDDDDD';};
    startGame(); const r0=[]; for(let i=0;i<20;i++)r0.push(rng());
    startGame(); r138HealthOf(S); r138HealthReviewHTML(); const r1=[]; for(let i=0;i<20;i++)r1.push(rng());
    let rngEq=true; for(let i=0;i<20;i++){ if(r0[i]!==r1[i]) rngEq=false; }
    out.rngZero = rngEq; randomSeedCode=oc;
    /* ③ deterministic 純讀終局 S：同態同結果、零 mutation、健康屬性確實驅動結局 */
    startGame(); S.attr={hp:20,int:75,apr:70,mny:60,hap:78}; S.age=45; S.flags={};
    const snap=JSON.stringify(S.attr)+'|'+S.age;
    const c1=r138HealthReviewHTML(), c2=r138HealthReviewHTML();
    out.deterministic = c1===c2 && c1.indexOf('健康人生')>=0
      && (JSON.stringify(S.attr)+'|'+S.age)===snap;
    /* ④ S=null/缺 attr 安全降級：回 null、卡空字串、不炸 */
    const sv=S; S=null; let nullSafe=true;
    try{ if(r138HealthOf(null)!==null) nullSafe=false; if(r138HealthReviewHTML()!=='') nullSafe=false; }catch(e){ nullSafe=false; }
    S=sv; out.nullSafe = nullSafe;
    return JSON.stringify(out);
  })()`, sandbox);
  const r138 = JSON.parse(r138Raw);
  r138OK = Object.values(r138).every(v => v === true);
  console.log(`R138 健康人生軌跡・病歷與長照: ${r138OK ? '✅ 全數通過' : '❌ ' + JSON.stringify(r138)}`);
} catch (e) {
  console.log('R138 健康人生軌跡・病歷與長照: ❌ ' + e.message);
}

/* ===== R139 學歷・職涯軌跡（純衍生/結算覆蓋層：零 rng/零 pinned 序列擾動/deterministic/智力主驅動/降級） ===== */
let r139OK = false;
try {
  const r139Raw = vm.runInContext(`(function(){
    const out={};
    /* ① 結構：6 學歷階層、6 職涯階層、6 結局型別、五個職涯成就＋集滿成就齊備 */
    out.struct = Array.isArray(R139_EDU) && R139_EDU.length===6 && Array.isArray(R139_CAREER) && R139_CAREER.length===6
      && Array.isArray(R139_PHASES) && R139_PHASES.length===4
      && ['freedom','manager','drudge','lieflat','layoff','lowwage'].every(k=>R139_END[k])
      && ['r139_freedom','r139_manager','r139_lieflat','r139_layoff','r139_lowwage','r139_career'].every(id=>!!ACH_MAP[id]);
    /* ② 純衍生覆蓋零 rng：startGame 後跑 r139CareerOf/Review 不消耗任何種子序列（與 pinned 抽事件序列獨立） */
    const oc=randomSeedCode; randomSeedCode=function(){return 'DDDDDD';};
    startGame(); const r0=[]; for(let i=0;i<20;i++)r0.push(rng());
    startGame(); r139CareerOf(S); r139CareerReviewHTML(); const r1=[]; for(let i=0;i<20;i++)r1.push(rng());
    let rngEq=true; for(let i=0;i<20;i++){ if(r0[i]!==r1[i]) rngEq=false; }
    out.rngZero = rngEq; randomSeedCode=oc;
    /* ③ deterministic 純讀終局 S：同態同結果、零 mutation、智力確實驅動學歷分檔 */
    startGame(); S.attr={hp:60,int:20,apr:55,mny:30,hap:60}; S.age=25; S.flags={};
    const snap=JSON.stringify(S.attr)+'|'+S.age;
    const c1=r139CareerReviewHTML(), c2=r139CareerReviewHTML();
    out.deterministic = c1===c2 && c1.indexOf('學歷')>=0 && c1.indexOf('職涯結局')>=0
      && (JSON.stringify(S.attr)+'|'+S.age)===snap;
    /* ④ 智力驅動學歷分檔：高智力→台清交/海歸(>=4)、低智力→放牛班(0) */
    const hi=r139CareerOf({attr:{int:95,mny:50},age:40,flags:{}}), lo=r139CareerOf({attr:{int:5,mny:50},age:40,flags:{}});
    out.intDrive = !!hi && !!lo && hi.eduTier>=4 && lo.eduTier===0 && hi.eduTier>lo.eduTier;
    /* ⑤ S=null/缺 attr/缺 int 安全降級：回 null、卡空字串、不炸 */
    const sv=S; S=null; let nullSafe=true;
    try{ if(r139CareerOf(null)!==null) nullSafe=false; if(r139CareerReviewHTML()!=='') nullSafe=false; if(r139CareerOf({attr:{},age:40})!==null) nullSafe=false; }catch(e){ nullSafe=false; }
    S=sv; out.nullSafe = nullSafe;
    return JSON.stringify(out);
  })()`, sandbox);
  const r139 = JSON.parse(r139Raw);
  r139OK = Object.values(r139).every(v => v === true);
  console.log(`R139 學歷・職涯軌跡: ${r139OK ? '✅ 全數通過' : '❌ ' + JSON.stringify(r139)}`);
} catch (e) {
  console.log('R139 學歷・職涯軌跡: ❌ ' + e.message);
}

/* ===== R140 婚戀家庭軌跡（純衍生/結算覆蓋層：零 rng/零 pinned 序列擾動/deterministic/魅力主驅動/降級） ===== */
let r140OK = false;
try {
  const r140Raw = vm.runInContext(`(function(){
    const out={};
    /* ① 結構：6 戀愛身價階層、6 婚戀關係階層、4 人生階段、6 結局型別、五個婚戀成就＋集滿成就齊備 */
    out.struct = Array.isArray(R140_CHARM) && R140_CHARM.length===6 && Array.isArray(R140_STATUS) && R140_STATUS.length===6
      && Array.isArray(R140_PHASES) && R140_PHASES.length===4
      && ['winner','couple','single','player','divorce','widow'].every(k=>R140_END[k])
      && ['r140_winner','r140_single','r140_player','r140_divorce','r140_widow','r140_love'].every(id=>!!ACH_MAP[id]);
    /* ② 純衍生覆蓋零 rng：startGame 後跑 r140LoveOf/Review 不消耗任何種子序列（與 pinned 抽事件序列獨立） */
    const oc=randomSeedCode; randomSeedCode=function(){return 'DDDDDD';};
    startGame(); const r0=[]; for(let i=0;i<20;i++)r0.push(rng());
    startGame(); r140LoveOf(S); r140LoveReviewHTML(); const r1=[]; for(let i=0;i<20;i++)r1.push(rng());
    let rngEq=true; for(let i=0;i<20;i++){ if(r0[i]!==r1[i]) rngEq=false; }
    out.rngZero = rngEq; randomSeedCode=oc;
    /* ③ deterministic 純讀終局 S：同態同結果、零 mutation、魅力確實驅動戀愛身價分檔 */
    startGame(); S.attr={hp:60,int:50,apr:20,mny:30,hap:60}; S.age=45; S.flags={};
    const snap=JSON.stringify(S.attr)+'|'+S.age;
    const c1=r140LoveReviewHTML(), c2=r140LoveReviewHTML();
    out.deterministic = c1===c2 && c1.indexOf('戀愛身價')>=0 && c1.indexOf('婚戀結局')>=0
      && (JSON.stringify(S.attr)+'|'+S.age)===snap;
    /* ④ 魅力驅動戀愛身價分檔：高魅力→萬人迷/海王(>=4)、低魅力→戀愛絕緣體(0) */
    const hi=r140LoveOf({attr:{apr:95,mny:50},age:40,flags:{}}), lo=r140LoveOf({attr:{apr:5,mny:50},age:40,flags:{}});
    out.charmDrive = !!hi && !!lo && hi.charmTier>=4 && lo.charmTier===0 && hi.charmTier>lo.charmTier;
    /* ⑤ S=null/缺 attr/缺 apr 安全降級：回 null、卡空字串、不炸 */
    const sv=S; S=null; let nullSafe=true;
    try{ if(r140LoveOf(null)!==null) nullSafe=false; if(r140LoveReviewHTML()!=='') nullSafe=false; if(r140LoveOf({attr:{},age:40})!==null) nullSafe=false; }catch(e){ nullSafe=false; }
    S=sv; out.nullSafe = nullSafe;
    return JSON.stringify(out);
  })()`, sandbox);
  const r140 = JSON.parse(r140Raw);
  r140OK = Object.values(r140).every(v => v === true);
  console.log(`R140 婚戀家庭軌跡: ${r140OK ? '✅ 全數通過' : '❌ ' + JSON.stringify(r140)}`);
} catch (e) {
  console.log('R140 婚戀家庭軌跡: ❌ ' + e.message);
}

/* ===== R141 興趣才藝・斜槓夢想軌跡（純衍生/結算覆蓋層：零 rng/零 pinned 序列擾動/deterministic/智力＋魅力主驅動/降級） ===== */
let r141OK = false;
try {
  const r141Raw = vm.runInContext(`(function(){
    const out={};
    /* ① 結構：6 才藝天賦階層、6 斜槓夢想階層、4 人生階段、5 結局型別、五個夢想成就＋集滿成就齊備 */
    out.struct = Array.isArray(R141_TALENT) && R141_TALENT.length===6 && Array.isArray(R141_DREAM) && R141_DREAM.length===6
      && Array.isArray(R141_PHASES) && R141_PHASES.length===4
      && ['slasher','viral','dreamer','quitter','couch'].every(k=>R141_END[k])
      && ['r141_slasher','r141_viral','r141_dreamer','r141_quitter','r141_couch','r141_hobby'].every(id=>!!ACH_MAP[id]);
    /* ② 純衍生覆蓋零 rng：startGame 後跑 r141HobbyOf/Review 不消耗任何種子序列（與 pinned 抽事件序列獨立） */
    const oc=randomSeedCode; randomSeedCode=function(){return 'DDDDDD';};
    startGame(); const r0=[]; for(let i=0;i<20;i++)r0.push(rng());
    startGame(); r141HobbyOf(S); r141HobbyReviewHTML(); const r1=[]; for(let i=0;i<20;i++)r1.push(rng());
    let rngEq=true; for(let i=0;i<20;i++){ if(r0[i]!==r1[i]) rngEq=false; }
    out.rngZero = rngEq; randomSeedCode=oc;
    /* ③ deterministic 純讀終局 S：同態同結果、零 mutation、智力＋魅力確實驅動才藝天賦分檔 */
    startGame(); S.attr={hp:60,int:50,apr:50,mny:30,hap:60}; S.age=45; S.flags={};
    const snap=JSON.stringify(S.attr)+'|'+S.age;
    const c1=r141HobbyReviewHTML(), c2=r141HobbyReviewHTML();
    out.deterministic = c1===c2 && c1.indexOf('才藝天賦')>=0 && c1.indexOf('夢想結局')>=0
      && (JSON.stringify(S.attr)+'|'+S.age)===snap;
    /* ④ 智力＋魅力雙屬性驅動才藝天賦分檔：雙高→全能斜槓王(>=4)、雙低→廢宅躺平(0) */
    const hi=r141HobbyOf({attr:{int:95,apr:95,mny:50},age:40,flags:{}}), lo=r141HobbyOf({attr:{int:5,apr:5,mny:50},age:40,flags:{}});
    out.talentDrive = !!hi && !!lo && hi.talentTier>=4 && lo.talentTier===0 && hi.talentTier>lo.talentTier;
    /* ⑤ S=null/缺 attr/缺 int 或 apr 安全降級：回 null、卡空字串、不炸 */
    const sv=S; S=null; let nullSafe=true;
    try{ if(r141HobbyOf(null)!==null) nullSafe=false; if(r141HobbyReviewHTML()!=='') nullSafe=false;
         if(r141HobbyOf({attr:{int:50},age:40})!==null) nullSafe=false; if(r141HobbyOf({attr:{apr:50},age:40})!==null) nullSafe=false; }catch(e){ nullSafe=false; }
    S=sv; out.nullSafe = nullSafe;
    return JSON.stringify(out);
  })()`, sandbox);
  const r141 = JSON.parse(r141Raw);
  r141OK = Object.values(r141).every(v => v === true);
  console.log(`R141 興趣才藝斜槓夢想軌跡: ${r141OK ? '✅ 全數通過' : '❌ ' + JSON.stringify(r141)}`);
} catch (e) {
  console.log('R141 興趣才藝斜槓夢想軌跡: ❌ ' + e.message);
}

/* ===== R142 理財投資・財富累積軌跡（純衍生/結算覆蓋層：零 rng/零 pinned 序列擾動/deterministic/財富＋智力主驅動/降級） ===== */
let r142OK = false;
try {
  const r142Raw = vm.runInContext(`(function(){
    const out={};
    /* ① 結構：6 投資智商階層、6 財富累積階層、4 人生階段、5 結局型別、五個投資成就＋集滿成就齊備 */
    out.struct = Array.isArray(R142_IQ) && R142_IQ.length===6 && Array.isArray(R142_PORT) && R142_PORT.length===6
      && Array.isArray(R142_PHASES) && R142_PHASES.length===4
      && ['fire','shipking','dividend','daytrader','leek'].every(k=>R142_END[k])
      && ['r142_fire','r142_shipking','r142_dividend','r142_daytrader','r142_leek','r142_invest'].every(id=>!!ACH_MAP[id]);
    /* ② 純衍生覆蓋零 rng：startGame 後跑 r142InvestOf/Review 不消耗任何種子序列（與 pinned 抽事件序列獨立） */
    const oc=randomSeedCode; randomSeedCode=function(){return 'EEEEEE';};
    startGame(); const r0=[]; for(let i=0;i<20;i++)r0.push(rng());
    startGame(); r142InvestOf(S); r142InvestReviewHTML(); const r1=[]; for(let i=0;i<20;i++)r1.push(rng());
    let rngEq=true; for(let i=0;i<20;i++){ if(r0[i]!==r1[i]) rngEq=false; }
    out.rngZero = rngEq; randomSeedCode=oc;
    /* ③ deterministic 純讀終局 S：同態同結果、零 mutation、財富＋智力確實驅動投資智商分檔 */
    startGame(); S.attr={hp:60,int:50,apr:50,mny:30,hap:60}; S.age=45; S.flags={};
    const snap=JSON.stringify(S.attr)+'|'+S.age;
    const c1=r142InvestReviewHTML(), c2=r142InvestReviewHTML();
    out.deterministic = c1===c2 && c1.indexOf('投資智商')>=0 && c1.indexOf('投資結局')>=0
      && (JSON.stringify(S.attr)+'|'+S.age)===snap;
    /* ④ 財富＋智力雙屬性驅動投資智商分檔：雙高→投資神之子(>=4)、雙低→月光族韭菜(0) */
    const hi=r142InvestOf({attr:{int:95,mny:95},age:40,flags:{}}), lo=r142InvestOf({attr:{int:5,mny:5},age:40,flags:{}});
    out.iqDrive = !!hi && !!lo && hi.iqTier>=4 && lo.iqTier===0 && hi.iqTier>lo.iqTier;
    /* ⑤ S=null/缺 attr/缺 mny 或 int 安全降級：回 null、卡空字串、不炸 */
    const sv=S; S=null; let nullSafe=true;
    try{ if(r142InvestOf(null)!==null) nullSafe=false; if(r142InvestReviewHTML()!=='') nullSafe=false;
         if(r142InvestOf({attr:{mny:50},age:40})!==null) nullSafe=false; if(r142InvestOf({attr:{int:50},age:40})!==null) nullSafe=false; }catch(e){ nullSafe=false; }
    S=sv; out.nullSafe = nullSafe;
    return JSON.stringify(out);
  })()`, sandbox);
  const r142 = JSON.parse(r142Raw);
  r142OK = Object.values(r142).every(v => v === true);
  console.log(`R142 理財投資財富累積軌跡: ${r142OK ? '✅ 全數通過' : '❌ ' + JSON.stringify(r142)}`);
} catch (e) {
  console.log('R142 理財投資財富累積軌跡: ❌ ' + e.message);
}

/* ===== R143 社交人脈・朋友圈軌跡（純衍生/結算覆蓋層：零 rng/零 pinned 序列擾動/deterministic/魅力＋智力主驅動/降級） ===== */
let r143OK = false;
try {
  const r143Raw = vm.runInContext(`(function(){
    const out={};
    /* ① 結構：6 社交手腕階層、6 朋友圈階層、4 人生階段、6 結局型別、六個社交成就＋集滿成就齊備 */
    out.struct = Array.isArray(R143_SOC) && R143_SOC.length===6 && Array.isArray(R143_CIRCLE) && R143_CIRCLE.length===6
      && Array.isArray(R143_PHASES) && R143_PHASES.length===4
      && ['maven','soulmate','boozer','fullstop','edge','lonely'].every(k=>R143_END[k])
      && ['r143_maven','r143_soulmate','r143_boozer','r143_fullstop','r143_edge','r143_lonely','r143_social'].every(id=>!!ACH_MAP[id]);
    /* ② 純衍生覆蓋零 rng：startGame 後跑 r143SocialOf/Review 不消耗任何種子序列（與 pinned 抽事件序列獨立） */
    const oc=randomSeedCode; randomSeedCode=function(){return 'EEEEEE';};
    startGame(); const r0=[]; for(let i=0;i<20;i++)r0.push(rng());
    startGame(); r143SocialOf(S); r143SocialReviewHTML(); const r1=[]; for(let i=0;i<20;i++)r1.push(rng());
    let rngEq=true; for(let i=0;i<20;i++){ if(r0[i]!==r1[i]) rngEq=false; }
    out.rngZero = rngEq; randomSeedCode=oc;
    /* ③ deterministic 純讀終局 S：同態同結果、零 mutation */
    startGame(); S.attr={hp:60,int:50,apr:50,mny:30,hap:60}; S.age=45; S.flags={};
    const snap=JSON.stringify(S.attr)+'|'+S.age;
    const c1=r143SocialReviewHTML(), c2=r143SocialReviewHTML();
    out.deterministic = c1===c2 && c1.indexOf('社交手腕')>=0 && c1.indexOf('社交結局')>=0
      && (JSON.stringify(S.attr)+'|'+S.age)===snap;
    /* ④ 魅力＋智力雙屬性驅動社交手腕分檔：雙高→交際花人脈王(>=4)、雙低→句點王(0) */
    const hi=r143SocialOf({attr:{int:95,apr:95,mny:50},age:40,flags:{}}), lo=r143SocialOf({attr:{int:5,apr:5,mny:50},age:40,flags:{}});
    out.socDrive = !!hi && !!lo && hi.socTier>=4 && lo.socTier===0 && hi.socTier>lo.socTier;
    /* ⑤ S=null/缺 attr/缺 apr 或 int 安全降級：回 null、卡空字串、不炸 */
    const sv=S; S=null; let nullSafe=true;
    try{ if(r143SocialOf(null)!==null) nullSafe=false; if(r143SocialReviewHTML()!=='') nullSafe=false;
         if(r143SocialOf({attr:{apr:50},age:40})!==null) nullSafe=false; if(r143SocialOf({attr:{int:50},age:40})!==null) nullSafe=false; }catch(e){ nullSafe=false; }
    S=sv; out.nullSafe = nullSafe;
    return JSON.stringify(out);
  })()`, sandbox);
  const r143 = JSON.parse(r143Raw);
  r143OK = Object.values(r143).every(v => v === true);
  console.log(`R143 社交人脈朋友圈軌跡: ${r143OK ? '✅ 全數通過' : '❌ ' + JSON.stringify(r143)}`);
} catch (e) {
...
/* ===== R145 居住・房產資產軌跡（純衍生/結算覆蓋層：零 rng/零 pinned 序列擾動/deterministic/財富 mny 主驅動+智力購屋眼光/降級） ===== */
let r145OK = false;
try {
  const r145Raw = vm.runInContext(`(function(){
    const out={};
    /* ① 結構：6 房產資產底子、6 居住處境階層、4 人生階段、6 結局型別、六個房產成就＋集滿成就齊備 */
    out.struct = Array.isArray(R145_ASSET) && R145_ASSET.length===6 && Array.isArray(R145_HOME) && R145_HOME.length===6
      && Array.isArray(R145_PHASES) && R145_PHASES.length===4
      && ['mansion','landlord','heir','mortgage','parasite','renter'].every(k=>R145_END[k])
      && ['r145_mansion','r145_landlord','r145_heir','r145_mortgage','r145_parasite','r145_renter','r145_housing'].every(id=>!!ACH_MAP[id]);
    /* ② 純衍生覆蓋零 rng：startGame 後跑 r145HousingOf/Review 不消耗任何種子序列（與 pinned 抽事件序列獨立） */
    const oc=randomSeedCode; randomSeedCode=function(){return 'EEEEEE';};
    startGame(); const r0=[]; for(let i=0;i<20;i++)r0.push(rng());
    startGame(); r145HousingOf(S); r145HousingReviewHTML(); const r1=[]; for(let i=0;i<20;i++)r1.push(rng());
    let rngEq=true; for(let i=0;i<20;i++){ if(r0[i]!==r1[i]) rngEq=false; }
    out.rngZero = rngEq; randomSeedCode=oc;
    /* ③ deterministic 純讀終局 S：同態同結果、零 mutation */
    startGame(); S.attr={hp:60,int:50,apr:50,mny:50,hap:60}; S.age=45; S.flags={};
    const snap=JSON.stringify(S.attr)+'|'+S.age;
    const c1=r145HousingReviewHTML(), c2=r145HousingReviewHTML();
    out.deterministic = c1===c2 && c1.indexOf('房產資產')>=0 && c1.indexOf('房產結局')>=0
      && (JSON.stringify(S.attr)+'|'+S.age)===snap;
    /* ④ 財富 mny 單調驅動資產底子：高 mny→豪宅置產大戶(>=4)、低 mny→零資產無殼(0) */
    const hi=r145HousingOf({attr:{mny:95,int:50},age:40,flags:{}}), lo=r145HousingOf({attr:{mny:5,int:50},age:40,flags:{}});
    out.mnyDrive = !!hi && !!lo && hi.assetTier>=4 && lo.assetTier===0 && hi.assetTier>lo.assetTier;
    /* ⑤ 六結局確定性分流互斥可達：帝寶/包租公/祖產/房貸/啃老/租屋各命中對應 endType */
    const E=(a)=>r145HousingOf(a)&&r145HousingOf(a).endType;
    out.endsReach =
         E({attr:{mny:88,int:50},age:40})==='mansion'
      && E({attr:{mny:65,int:60},age:40})==='landlord'
      && E({attr:{mny:65,int:50},age:25})==='heir'
      && E({attr:{mny:50,int:50},age:40})==='mortgage'
      && E({attr:{mny:20,int:50},age:28})==='parasite'
      && E({attr:{mny:20,int:50},age:66})==='renter';
    /* ⑥ S=null/缺 attr/缺 mny 安全降級：回 null、卡空字串、不炸 */
    const sv=S; S=null; let nullSafe=true;
    try{ if(r145HousingOf(null)!==null) nullSafe=false; if(r145HousingReviewHTML()!=='') nullSafe=false;
         if(r145HousingOf({attr:{int:50},age:40})!==null) nullSafe=false; }catch(e){ nullSafe=false; }
    S=sv; out.nullSafe = nullSafe;
    return JSON.stringify(out);
  })()`, sandbox);
  const r145 = JSON.parse(r145Raw);
  r145OK = Object.values(r145).every(v => v === true);
  console.log(`R145 居住房產資產軌跡: ${r145OK ? '✅ 全數通過' : '❌ ' + JSON.stringify(r145)}`);
} catch (e) {
  console.log('R145 居住房產資產軌跡: ❌ ' + e.message);
}

/* ===== R146 交通・座駕軌跡（純衍生/結算覆蓋層：零 rng/零 pinned 序列擾動/deterministic/財富 mny 主驅動+智力購車眼光/降級） ===== */
let r146OK = false;
try {
  const r146Raw = vm.runInContext(`(function(){
    const out={};
    /* ① 結構：6 座駕資產底子、6 通勤處境階層、4 人生階段、6 結局型別、六個交通成就＋集滿成就齊備 */
    out.struct = Array.isArray(R146_FLEET) && R146_FLEET.length===6 && Array.isArray(R146_RIDE) && R146_RIDE.length===6
      && Array.isArray(R146_PHASES) && R146_PHASES.length===4
      && ['luxe','biker','gogoro','toyota','scooter','walker'].every(k=>R146_END[k])
      && ['r146_luxe','r146_biker','r146_gogoro','r146_toyota','r146_scooter','r146_walker','r146_vehicle'].every(id=>!!ACH_MAP[id]);
    /* ② 純衍生覆蓋零 rng：startGame 後跑 r146VehicleOf/Review 不消耗任何種子序列（與 pinned 抽事件序列獨立） */
    const oc=randomSeedCode; randomSeedCode=function(){return 'EEEEEE';};
    startGame(); const r0=[]; for(let i=0;i<20;i++)r0.push(rng());
    startGame(); r146VehicleOf(S); r146VehicleReviewHTML(); const r1=[]; for(let i=0;i<20;i++)r1.push(rng());
    let rngEq=true; for(let i=0;i<20;i++){ if(r0[i]!==r1[i]) rngEq=false; }
    out.rngZero = rngEq; randomSeedCode=oc;
    /* ③ deterministic 純讀終局 S：同態同結果、零 mutation */
    startGame(); S.attr={hp:60,int:50,apr:50,mny:50,hap:60}; S.age=45; S.flags={};
    const snap=JSON.stringify(S.attr)+'|'+S.age;
    const c1=r146VehicleReviewHTML(), c2=r146VehicleReviewHTML();
    out.deterministic = c1===c2 && c1.indexOf('座駕資產')>=0 && c1.indexOf('交通結局')>=0
      && (JSON.stringify(S.attr)+'|'+S.age)===snap;
    /* ④ 財富 mny 單調驅動座駕底子：高 mny→雙B進口車庫(>=4)、低 mny→無車雙腳族(0) */
    const hi=r146VehicleOf({attr:{mny:95,int:50},age:40,flags:{}}), lo=r146VehicleOf({attr:{mny:5,int:50},age:40,flags:{}});
    out.mnyDrive = !!hi && !!lo && hi.fleetTier>=4 && lo.fleetTier===0 && hi.fleetTier>lo.fleetTier;
    /* ⑤ 六結局確定性分流互斥可達：雙B/重機/Gogoro/Toyota/機車/無車各命中對應 endType */
    const E=(a)=>r146VehicleOf(a)&&r146VehicleOf(a).endType;
    out.endsReach =
         E({attr:{mny:88,int:50},age:40})==='luxe'
      && E({attr:{mny:65,int:50},age:25})==='biker'
      && E({attr:{mny:65,int:60},age:40})==='gogoro'
      && E({attr:{mny:50,int:50},age:40})==='toyota'
      && E({attr:{mny:20,int:50},age:28})==='scooter'
      && E({attr:{mny:20,int:50},age:66})==='walker';
    /* ⑥ S=null/缺 attr/缺 mny 安全降級：回 null、卡空字串、不炸 */
    const sv=S; S=null; let nullSafe=true;
    try{ if(r146VehicleOf(null)!==null) nullSafe=false; if(r146VehicleReviewHTML()!=='') nullSafe=false;
         if(r146VehicleOf({attr:{int:50},age:40})!==null) nullSafe=false; }catch(e){ nullSafe=false; }
    S=sv; out.nullSafe = nullSafe;
    return JSON.stringify(out);
  })()`, sandbox);
  const r146 = JSON.parse(r146Raw);
  r146OK = Object.values(r146).every(v => v === true);
  console.log(`R146 交通座駕軌跡: ${r146OK ? '✅ 全數通過' : '❌ ' + JSON.stringify(r146)}`);
} catch (e) {
...
/* ===== R153 數位・網路生活軌跡（純衍生/結算覆蓋層：零 rng/零 pinned 序列擾動/deterministic/四屬性合成網路分+智力主驅動/降級） ===== */
let r153OK = false;
try {
  const r153Raw = vm.runInContext(`(function(){
    const out={};
    /* ① 結構：8 級網路命運階梯、4 人生階段、8 結局型別、八個網路成就＋集滿成就齊備 */
    out.struct = Array.isArray(R153_NET) && R153_NET.length===8
      && Array.isArray(R153_PHASES) && R153_PHASES.length===4
      && ['offline','boomer','fbspam','pttkbd','dcard','tipper','netred','kol'].every(k=>R153_END[k])
      && ['r153_kol','r153_netred','r153_tipper','r153_dcard','r153_pttkbd','r153_fbspam','r153_boomer','r153_offline','r153_net'].every(id=>!!ACH_MAP[id]);
    /* ② 純衍生覆蓋零 rng：startGame 後跑 r153NetOf/Review 不消耗任何種子序列（與 pinned 抽事件序列獨立） */
    const oc=randomSeedCode; randomSeedCode=function(){return 'EEEEEE';};
    startGame(); const r0=[]; for(let i=0;i<20;i++)r0.push(rng());
    startGame(); r153NetOf(S); r153NetReviewHTML(); const r1=[]; for(let i=0;i<20;i++)r1.push(rng());
    let rngEq=true; for(let i=0;i<20;i++){ if(r0[i]!==r1[i]) rngEq=false; }
    out.rngZero = rngEq; randomSeedCode=oc;
    /* ③ deterministic 純讀終局 S：同態同結果、零 mutation */
    startGame(); S.attr={hp:60,int:55,apr:55,mny:55,hap:60}; S.age=45; S.flags={};
    const snap=JSON.stringify(S.attr)+'|'+S.age;
    const c1=r153NetReviewHTML(), c2=r153NetReviewHTML();
    out.deterministic = c1===c2 && c1.indexOf('網路底子')>=0 && c1.indexOf('網路結局')>=0
      && (JSON.stringify(S.attr)+'|'+S.age)===snap;
    /* ④ 智力 int 為主單調驅動網路分/定級：高 int→KOL 段(tier>=6)、低 int→墊底(tier<=1) */
    const hi=r153NetOf({attr:{int:95,apr:95,mny:95,hp:95},age:45,flags:{}}), lo=r153NetOf({attr:{int:5,apr:5,mny:5,hp:5},age:45,flags:{}});
    out.intDrive = !!hi && !!lo && hi.tier>=6 && lo.tier<=1 && hi.tier>lo.tier;
    /* ⑤ 八結局確定性分流互斥可達：四屬性等值（無 era → gen 50 中性）掃 0..7 各命中對應 endType（age 45 解鎖全上限） */
    const E=(v)=>{const r=r153NetOf({attr:{int:v,apr:v,mny:v,hp:v},age:45}); return r&&r.endType;};
    out.endsReach =
         E(5)==='offline' && E(16)==='boomer' && E(26)==='fbspam' && E(40)==='pttkbd'
      && E(54)==='dcard' && E(68)==='tipper' && E(80)==='netred' && E(95)==='kol';
    /* ⑥ S=null/缺 attr/缺 int 安全降級：回 null、卡空字串、不炸 */
    const sv=S; S=null; let nullSafe=true;
    try{ if(r153NetOf(null)!==null) nullSafe=false; if(r153NetReviewHTML()!=='') nullSafe=false;
         if(r153NetOf({attr:{apr:50},age:40})!==null) nullSafe=false; }catch(e){ nullSafe=false; }
    S=sv; out.nullSafe = nullSafe;
    return JSON.stringify(out);
  })()`, sandbox);
  const r153 = JSON.parse(r153Raw);
  r153OK = Object.values(r153).every(v => v === true);
  console.log(`R153 數位網路生活軌跡: ${r153OK ? '✅ 全數通過' : '❌ ' + JSON.stringify(r153)}`);
} catch (e) {
